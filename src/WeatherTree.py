...
def init_servo(pwm_pin=servo_pwm_pin, pwm_frequency=servo_pwm_frequency):
    servo = Servo(
        pwm_pin, pwm_frequency, 0, max_servo_angle, min_servo_duty, max_servo_duty
    )
    return servo
...
def init_lcd(sda_pin=lcd_sda_pin, scl_pin=lcd_scl_pin):
    i2c = I2C(
        0, sda=machine.Pin(int(sda_pin)), scl=machine.Pin(int(scl_pin)), freq=400000
    )
...
def main():
    ...
    animation = Animation(
        led_number_of_leds,
        animation_snow_grouping,
        animation_rain_dribble,
        animation_thunderness,
    )
    ...
    while True:  # demo_mode == 0:
        # Fetch the weather
        current_city = cities[city_index]
        temp, wind_dir, condition, precipitation = weather.get_city_weather(
            current_city
        )
        ...
        if abs(last_frame - cur_time) > led_frame_period:
            ...
            if condition.lower().find("thunder") != -1:
                ...
                for led in range(len(led_colours)):
                    cur_colour = led_strip.colorHSV(
                        led_colours[led][0], led_colours[led][1], led_colours[led][2]
                    )
